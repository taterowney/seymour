...
structure DisjointCircuitFamily {α : Type} (M : Matroid α) where
  /-- Indexing set -/
  Idx : Set α
...
def DisjointCircuitFamily.union {α : Type} {M : Matroid α} (F : DisjointCircuitFamily M) : Set α :=
  Set.iUnion F.F

/-- Every element in `DisjointCircuitFamily` is subset of ground set. -/
lemma DisjointCircuitFamily.mem_subset_ground {α : Type} {M : Matroid α} (F : DisjointCircuitFamily M)
    (x : F.Idx) : F.F x ⊆ M.E := (F.hCircuit x).subset_ground

/-- Union of sets in `DisjointCircuitFamily` is subset of ground set. -/
lemma DisjointCircuitFamily.union_subset_ground {α : Type} {M : Matroid α} (F : DisjointCircuitFamily M) :
    F.union ⊆ M.E := by
...
lemma DisjointCircuitFamily.union_indep_empty {α : Type} {M : Matroid α} (F : DisjointCircuitFamily M) :
...
lemma DisjointCircuitFamily.union_nonempty_dep {α : Type} {M : Matroid α} (F : DisjointCircuitFamily M) :
...
lemma DisjointCircuitFamily.dep_or_empty {α : Type} {M : Matroid α} (F : DisjointCircuitFamily M) :
