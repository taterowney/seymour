...
def DeltaSum.CircuitForm {α : Type} [DecidableEq α] (M₁ M₂ : BinaryMatroid α) (C : Set α) : Prop :=
  C.Nonempty ∧ C ⊆ DeltaSum.E M₁ M₂ ∧
    ∃ X₁, ∃ X₂, X₁ ⊆ M₁.E ∧ X₂ ⊆ M₂.E ∧ C = symmDiff X₁ X₂ ∧
    M₁.matroid.UnionDisjointCircuits X₁ ∧ M₂.matroid.UnionDisjointCircuits X₂
...
@[simp]
lemma DeltaSum.CircuitPred_iff {α : Type} [DecidableEq α] (M₁ M₂ : BinaryMatroid α) :
  (DeltaSum.CircuitMatroid M₁ M₂).CircuitPred = DeltaSum.CircuitPred M₁ M₂ := rfl
...
lemma DeltaSum.CircuitPred_left_circuit {α : Type} [DecidableEq α] {M₁ M₂ : BinaryMatroid α} {C : Set α}
  ...
  · intro D ⟨hDnempty, hD⟩ hDC
    ...
    have t1 : C ⊆ M₁.E := sub_union_diff_sub_union hCE
    have t2 : D ⊆ M₁.E \ M₂.E := hDC.trans hCE
    have t3 : Disjoint (M₁.E \ M₂.E) M₂.E := Set.disjoint_sdiff_left
    have t4 : Disjoint D X₂ := Set.disjoint_of_subset t2 hX₂E t3

    have t5a : D ⊆ M₁.E := hDC.trans t1
    have t5 : X₂ ⊆ M₁.E := symmDiff_subset_parent_left (hDX₁X₂ ▸ t5a) hX₁E

    have t := hCM₁.2
    sorry

/-- Circuit of `M₂` that lies in `M₂.E \ M₁.E` is circuit in `M₁ Δ M₂`. -/
lemma DeltaSum.CircuitPred_right_circuit {α : Type} [DecidableEq α] {M₁ M₂ : BinaryMatroid α} {C : Set α}
    (hCM₂ : M₂.matroid.Circuit C) (hCE : C ⊆ M₂.E \ M₁.E) : DeltaSum.CircuitPred M₁ M₂ C := by
  sorry
...
lemma DeltaSum.Disjoint_circuit_left_circuit {α : Type} [DecidableEq α] {M₁ M₂ : BinaryMatroid α}
  ...
  · intro C' hC' hC'subC
    if hCC' : C' = C then
      ...
      have hC'M₁ : C' ⊆ M₁.E := Set.Subset.trans hC'subC hC.subset_ground
      have hC'M₂ : Disjoint C' M₂.E := Set.disjoint_of_subset hC'M₁ (Set.Subset.refl M₂.E) hM₁M₂
      -- ^ question: fix false flag "unused variable"?
      have hX₁X₂ : Disjoint X₁ X₂ := Set.disjoint_of_subset hX₁ hX₂ hM₁M₂
      rw [Disjoint.symmDiff_eq_sup hX₁X₂] at hC'X₁X₂

      have hX₂C' : X₂ ⊆ C' := Set.Subset.trans Set.subset_union_right hC'X₁X₂.symm.subset
      have hX₂empty : X₂ = ∅ := Set.subset_eq_empty (hC'M₂ hX₂C' hX₂) rfl
      simp only [Set.sup_eq_union] at hC'X₁X₂
      rw [hX₂empty, Set.union_empty] at hC'X₁X₂

      have hC'C : C' ⊂ C := HasSubset.Subset.ssubset_of_ne hC'subC hCC'
      have hC'indep := Matroid.Circuit.indep_ssub hC hC'C
      have hC'empty : C' = ∅ := Matroid.UnionDisjointCircuits.indep_empty (hC'X₁X₂ ▸ hX₁M₁) hC'indep
      exfalso
...
lemma DeltaSum.Disjoint_circuit_right_circuit {α : Type} [DecidableEq α] {M₁ M₂ : BinaryMatroid α}
  ...
  · intro C' hC' hC'subC
    if hCC' : C' = C then
      ...
      obtain ⟨hC'nonempty, hC'E, X₁, X₂, hX₁, hX₂, hC'X₁X₂, hX₁M₁, hX₂M₂⟩ := hC'
      have hC'M₂ : C' ⊆ M₂.E := Set.Subset.trans hC'subC hC.subset_ground
      have hC'M₁ : Disjoint C' M₁.E := Set.disjoint_of_subset hC'M₂ (Set.Subset.refl M₁.E) hM₁M₂.symm
      -- ^ question: fix false flag "unused variable"?
      have hX₁X₂ : Disjoint X₁ X₂ := Set.disjoint_of_subset hX₁ hX₂ hM₁M₂
      rw [Disjoint.symmDiff_eq_sup hX₁X₂] at hC'X₁X₂

      have hX₁C' : X₁ ⊆ C' := Set.Subset.trans Set.subset_union_left hC'X₁X₂.symm.subset
      have hX₁empty : X₁ = ∅ := Set.subset_eq_empty (hC'M₁ hX₁C' hX₁) rfl
      simp only [Set.sup_eq_union] at hC'X₁X₂
...
lemma DeltaSum.SpecialCase1Sum {α : Type} [DecidableEq α] {M₁ M₂ : BinaryMatroid α}
    (hM₁M₂ : Disjoint M₁.E M₂.E) : Matroid.disjointSum M₁.matroid M₂.matroid hM₁M₂ = DeltaSum.matroid M₁ M₂ := by
    ...
    · intro ⟨⟨hCnempty, hCE, X₁, X₂, hX₁, hX₂, hCX₁X₂, hX₁duc, hX₂duc⟩, hCmin⟩
      ...
      if hX₁empty : X₁ = ∅ then
        ...
        · intro D hD hDX₂
          ...
          have hD'form : DeltaSum.CircuitForm M₁ M₂ D' := DeltaSum.CircuitForm_circuit_right hD'
        ...
        · intro D hD hDX₂
          have ⟨D', hD', hD'D⟩ := Matroid.Circuit.dep_iff_has_circuit.mp hD
          have hD'form : DeltaSum.CircuitForm M₁ M₂ D' := DeltaSum.CircuitForm_circuit_left hD'
...
lemma DeltaSum.SpecialCase2Sum_CircuitPred {α : Type} [DecidableEq α] {M₁ M₂ : BinaryMatroid α}
    (Assumptions : Matroid.TwoSum.Assumptions M₁.matroid M₂.matroid) :
    (Matroid.TwoSum.CircuitMatroid Assumptions).CircuitPred = (DeltaSum.CircuitMatroid M₁ M₂).CircuitPred := by
  sorry
  -- rw [CircuitMatroid.CircuitPred_eq_iff, DeltaSum.CircuitPred_iff, Matroid.TwoSum.CircuitPred_iff]
  ...
  --   cases hC with
  --   | inl hC =>
  --       apply DeltaSum.CircuitPred_left_circuit
  --       exact hC.2
  --       have t := disjoint_of_singleton_inter_subset_left Assumptions.hp hC.2.subset_ground hC.1
  --       exact Set.subset_diff.mpr ⟨hC.2.subset_ground, t⟩
  --   | inr hC =>
  --       cases hC with
  --       | inl hC =>
  --           apply DeltaSum.CircuitPred_right_circuit
  --           exact hC.2
  --           have t :=  disjoint_of_singleton_inter_subset_right Assumptions.hp hC.2.subset_ground hC.1
  --           exact Set.subset_diff.mpr ⟨hC.2.subset_ground, t.symm⟩
  --       | inr hC =>
  --           have ⟨hCE, hCM₁, hCM₂⟩ := hC
  --           constructor
  --           · constructor
...
lemma DeltaSum.SpecialCase2Sum {α : Type} [DecidableEq α] {M₁ M₂ : BinaryMatroid α}
    (Assumptions : Matroid.TwoSum.Assumptions M₁.matroid M₂.matroid) :
