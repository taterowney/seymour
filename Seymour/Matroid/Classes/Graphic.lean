...
lemma Matroid.IsRepresentedBy.isTotallyUnimodular_of_isGraphic {α X : Type} [DecidableEq α] {M : Matroid α}
    {A : Matrix X M.E ℚ} (hMA : M.IsRepresentedBy A) (hA : A.IsGraphic) :
    A.IsTotallyUnimodular := by
  sorry

/-- Graphic matroid is regular. -/
lemma Matroid.IsGraphic.isRegular {α : Type} [DecidableEq α] {M : Matroid α} (hM : M.IsGraphic) : M.IsRegular := by
...
lemma Matroid.IsCographic.is_regular {α : Type} [DecidableEq α] {M : Matroid α} (hM : M.IsCographic) : M.IsRegular :=
