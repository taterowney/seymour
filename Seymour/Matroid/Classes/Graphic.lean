...
def Matroid.IsGraphic {α : Type} [DecidableEq α] (M : Matroid α) [Fintype M.E] : Prop :=
  ∃ X : Type, ∃ A : Matrix X M.E ℚ, M.IsRepresentedBy A ∧ A.IsGraphic

/-- Graphic matroid can be represented only by a TU matrix. -/
lemma Matroid.IsRepresentedBy.isTotallyUnimodular_of_isGraphic {α X : Type} [DecidableEq α] {M : Matroid α} [Fintype M.E]
    {A : Matrix X M.E ℚ} (hMA : M.IsRepresentedBy A) (hA : A.IsGraphic) :
    A.IsTotallyUnimodular := by
  sorry

/-- Graphic matroid is regular. -/
lemma Matroid.IsGraphic.isRegular {α : Type} [DecidableEq α] {M : Matroid α} [Fintype M.E] (hM : M.IsGraphic) :
    M.IsRegular := by
...
-- TODO `instance` for `Fintype M✶.E`

/-- Matroid is cographic iff its dual is represented by an incidence matrix of a graph. -/
def Matroid.IsCographic {α : Type} [DecidableEq α] (M : Matroid α) [Fintype M✶.E] : Prop :=
  M✶.IsGraphic

/-- Cographic matroid is regular. -/
lemma Matroid.IsCographic.is_regular {α : Type} [DecidableEq α] {M : Matroid α} [Fintype M✶.E] (hM : M.IsCographic) :
    M.IsRegular :=
