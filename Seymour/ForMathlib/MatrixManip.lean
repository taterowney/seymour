...
/-- todo: desc -/
def Matrix.castColsSetUnion (A : Matrix X (Y₁ ∪ Y₂).Elem R) (hY : Y₁ ∪ Y₂ = Y₀) : Matrix X Y₀ R :=
...
def Matrix.fromColsSetUnion [∀ a, Decidable (a ∈ Y₁)] [∀ a, Decidable (a ∈ Y₂)] (A₁ : Matrix X Y₁ R) (A₂ : Matrix X Y₂ R) :
    Matrix X (Y₁ ∪ Y₂).Elem R :=
...
def Matrix.toColsSetUnion₁ (A : Matrix X (Y₁ ∪ Y₂).Elem R) : Matrix X Y₁ R :=
...
/-- todo: desc -/
def Matrix.castRowsSetUnion (A : Matrix (X₁ ∪ X₂).Elem Y R) (hX : X₁ ∪ X₂ = X₀) : Matrix X₀ Y R :=
...
def Matrix.fromRowsSetUnion [∀ a, Decidable (a ∈ X₁)] [∀ a, Decidable (a ∈ X₂)] (A₁ : Matrix X₁ Y R) (A₂ : Matrix X₂ Y R) :
    Matrix (X₁ ∪ X₂).Elem Y R :=
  Matrix.of fun i j => (Matrix.fromRows A₁ A₂) i.toSum j

/-- Given a row partitioned matrix extract the first row -/
def Matrix.toRowsSetUnion₁ (A : Matrix (X₁ ∪ X₂).Elem Y R) : Matrix X₁ Y R :=
...
def Matrix.fromBlocksSetUnion' {T₁ T₂ : Type}
  ...
  Matrix.fromRows (Matrix.fromColsSetUnion A B) (Matrix.fromColsSetUnion C D)
